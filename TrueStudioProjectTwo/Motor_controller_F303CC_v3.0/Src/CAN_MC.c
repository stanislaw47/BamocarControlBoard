...
void CAN_MC_Transmit(uint8_t d1, uint8_t d2, uint8_t d3){
	CanTxMsgTypeDef Frame = TxMessage;
	Frame.Data[0] = d1;
	Frame.Data[1] = d2;
	Frame.Data[2] = d3;
...
void CAN_MC_Init(void){
	...
	hcan.pTxMsg = &TxMessageBufor; //pass this structure to main structure of CAN handler
...
void CAN_MC_SendRTD(uint8_t RTD){
	if(RTD == 1 & RTDprev == 0)
		{
		TxMessageBufor.StdId = 0x150;
		TxMessageBufor.DLC = 1;
		TxMessageBufor.Data[0]=RTD;
		hcan.pTxMsg = &TxMessageBufor;
